...
class TestTimeSeriesCalculator(unittest.TestCase):
    ...
    def test_safe_divide(self):
        ...
        self.assertEqual(result.iloc[1], 0.0)  # 20/0 -> 0 (default)
        self.assertEqual(result.iloc[2], 6.0)  # 30/5
        
        # default=np.nan 时，除零应为NaN
        result_nan = TimeSeriesCalculator.safe_divide(numerator, denominator, default=np.nan)
        self.assertTrue(pd.isna(result_nan.iloc[1]))  # 20/0 -> NaN
