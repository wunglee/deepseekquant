...
class WindowManagerTest(unittest.TestCase):
    ...
    def test_make_window_key_monthly(self):
        ...
        key = self.mgr.make_window_key(date, 'monthly', window_size=1)
        self.assertEqual(key, '2025-03_03')
    
    def test_make_window_key_weekly(self):
        """测试周度窗口键生成"""
        date = pd.Timestamp('2025-03-15')  # 2025年第10周
        key = self.mgr.make_window_key(date, 'weekly', window_size=1)
    ...
    def test_generate_window_keys_single_month(self):
        """测试生成单月窗口键"""
        keys = self.mgr.generate_window_keys('2025-01-01', '2025-01-31', 'monthly', window_size=1)
        self.assertEqual(len(keys), 1)
        self.assertEqual(keys[0], '2025-01_01')
    
    def test_generate_window_keys_multiple_months(self):
        """测试生成多月窗口键"""
        keys = self.mgr.generate_window_keys('2025-01-01', '2025-03-31', 'monthly', window_size=1)
        self.assertEqual(len(keys), 3)
        self.assertEqual(keys, ['2025-01_01', '2025-02_02', '2025-03_03'])
    
    def test_generate_window_keys_cross_year(self):
        """测试生成跨年窗口键"""
        keys = self.mgr.generate_window_keys('2024-11-01', '2025-02-28', 'monthly', window_size=1)
        self.assertEqual(len(keys), 4)
        self.assertEqual(keys, ['2024-11_11', '2024-12_12', '2025-01_01', '2025-02_02'])
    
    def test_generate_window_keys_weekly(self):
        """测试生成周度窗口键"""
        keys = self.mgr.generate_window_keys('2025-01-01', '2025-01-31', 'weekly', window_size=1)
        # 1月通常包含4-5周
    ...
    def test_generate_window_keys_leap_year_february(self):
        """测试闰年2月窗口键"""
        # 2024是闰年
        keys = self.mgr.generate_window_keys('2024-02-01', '2024-02-29', 'monthly', window_size=1)
        self.assertEqual(len(keys), 1)
        self.assertEqual(keys[0], '2024-02_02')
        
        # 验证2月29日包含在内
        keys_daily = self.mgr.generate_window_keys('2024-02-28', '2024-02-29', 'daily', window_size=1)
    ...
    def test_generate_window_keys_month_end_boundaries(self):
        """测试月末边界场景"""
        # 31天的月
        keys = self.mgr.generate_window_keys('2025-01-31', '2025-02-01', 'monthly', window_size=1)
        self.assertEqual(len(keys), 2)
        
        # 30天的月
        keys = self.mgr.generate_window_keys('2025-04-30', '2025-05-01', 'monthly', window_size=1)
        self.assertEqual(len(keys), 2)
    
    def test_generate_window_keys_year_boundary(self):
        """测试年末边界场景"""
        keys = self.mgr.generate_window_keys('2024-12-31', '2025-01-01', 'monthly', window_size=1)
        self.assertEqual(len(keys), 2)
    ...
    def test_window_key_to_date_monthly_range(self):
        """测试月度窗口键转为日期范围"""
        start, end = self.mgr.window_key_to_date_range('2025-03_03', 'monthly')
    ...
    def test_window_boundary_monthly_consecutive(self):
        """测试连续月份的窗口边界"""
        _, end1 = self.mgr.window_key_to_date_range('2025-01_01', 'monthly')
        start2, _ = self.mgr.window_key_to_date_range('2025-02_02', 'monthly')
    ...
    def test_window_boundary_daily_consecutive(self):
        """测试连续日期的窗口边界"""
        _, end1 = self.mgr.window_key_to_date_range('20250115_20250115', 'daily')
        start2, _ = self.mgr.window_key_to_date_range('20250116_20250116', 'daily')
    ...
    def test_generate_window_keys_invalid_date_range(self):
        """测试无效日期范围（开始日期晚于结束日期）"""
        keys = self.mgr.generate_window_keys('2025-03-01', '2025-01-01', 'monthly', window_size=1)
        # 应返回空列表或抛出异常（取决于实现）
        self.assertEqual(len(keys), 0)
    
    def test_generate_window_keys_same_date(self):
        """测试相同日期"""
        keys = self.mgr.generate_window_keys('2025-01-15', '2025-01-15', 'daily', window_size=1)
        self.assertEqual(len(keys), 1)
    ...
    def test_window_key_to_date_invalid_format(self):
        ...
        try:
            result = self.mgr.window_key_to_date_range('invalid-key', 'monthly')
