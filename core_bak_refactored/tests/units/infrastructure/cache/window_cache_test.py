...
class TestWindowsCacheBasic(unittest.TestCase):
    ...
    def test_make_window_key_daily(self):
        ...
        window_key = self.window_cache._make_window_key(date, 'daily', self.market_code)
    ...
    def test_make_window_key_weekly(self):
        """测试生成周窗口键"""
        date = pd.Timestamp('2025-01-15')
        window_key = self.window_cache._make_window_key(date, 'weekly', self.market_code)
    ...
    def test_make_window_key_monthly(self):
        ...
        window_key = self.window_cache._make_window_key(date, 'monthly', self.market_code)
...
class TestWindowsCachePutGet(unittest.TestCase):
    ...
    def test_put_and_get_window_data(self):
        ...
        # 生成窗口键
        window_key = self.window_cache._make_window_key(date, period, self.market_code)
...
class TestMergeWindows(unittest.TestCase):
    ...
    def test_merge_continuous_windows_empty_list(self):
        """测试空窗口列表"""
        result = self.window_cache.merge_continuous_windows([], 'daily', self.market_code)
    ...
    def test_merge_continuous_windows_all_consecutive(self):
        ...
        result = self.window_cache.merge_continuous_windows(keys, 'weekly', self.market_code)
    ...
    def test_merge_continuous_windows_with_gap(self):
        ...
        result = self.window_cache.merge_continuous_windows(keys, 'monthly', self.market_code)
...
class TestGetCachedAndMissingWindows(unittest.TestCase):
    ...
    def test_current_window_refresh(self):
        ...
        # 写入当前窗口的缓存
        current_key = self.window_cache._make_window_key(current_time, 'daily', self.market_code)
        if current_key:
            test_data = pd.DataFrame({
                'date': [start_date],
                'close': [100.0]
    ...
    def test_first_window_filtering(self):
        ...
        first_window_key = self.window_cache._make_window_key(
            pd.Timestamp('2025-01-13'), 'daily', self.market_code
        )
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_make_window_key_type_error_date(self):
        ...
        with self.assertRaises(TypeError):
            self.window_cache._make_window_key('2025-01-15', 'daily', self.market_code)
    
    def test_make_window_key_type_error_market_code(self):
        """测试非MarketCode类型的market_code参数"""
        with self.assertRaises(TypeError):
            self.window_cache._make_window_key('2025-01-15', 'daily', MarketCode.CN)
    
    def test_make_window_key_invalid_period(self):
        """测试无效的周期类型"""
        with self.assertRaises(ValueError):
            self.window_cache._make_window_key(
                pd.Timestamp('2025-01-15'), 'hourly', self.market_code
            )
    ...
    def test_make_window_key_returns_none_for_invalid_window(self):
        ...
        result = self.window_cache._make_window_key(date, 'daily', self.market_code)
