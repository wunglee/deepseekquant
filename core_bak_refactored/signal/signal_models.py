"""
信号引擎系统 - 枚举和数据模型
拆分自: core_bak/signal_engine.py (line 42-182)
职责: 定义信号相关的所有枚举和数据类
"""
...
class TradingSignal:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'TradingSignal':
        ...
        if 'source' in metadata_data and isinstance(metadata_data['source'], str):
            metadata_data['source'] = SignalSource(metadata_data['source'])
        if 'strength' in metadata_data and isinstance(metadata_data['strength'], str):
            metadata_data['strength'] = SignalStrength(metadata_data['strength'])
