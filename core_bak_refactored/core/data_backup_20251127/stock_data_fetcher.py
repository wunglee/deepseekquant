...
import numpy as np
import pytz
from .data_models import MarketData, DataSourceType
    ...
    async def _fetch_alpha_vantage_data(self, symbol: str, period: str, interval: str,
        ...
        try:
            ...
            function_map = {
                ...
                '1d': 'TIME_SERIES_DAILY_ADJUSTED' if adjustments else 'TIME_SERIES_DAILY',
                '1wk': 'TIME_SERIES_WEEKLY_ADJUSTED' if adjustments else 'TIME_SERIES_WEEKLY',
                '1mo': 'TIME_SERIES_MONTHLY_ADJUSTED' if adjustments else 'TIME_SERIES_MONTHLY',
            }
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(url, params=params) as response:
                        if response.status != 200:
                            logger.error(f"Alpha Vantage API 调用失败: {response.status}")
                            return None
                        data = await response.json()
            if 'Error Message' in data or 'Note' in data:
                logger.warning(f"Alpha Vantage 返回错误: {data}")
                return None
            ...
            if not time_series_key:
                logger.error(f"Alpha Vantage 数据格式异常: {list(data.keys())}")
                return None
