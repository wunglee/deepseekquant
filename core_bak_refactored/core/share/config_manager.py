...
class ConfigManager:
    ...
    def get_cache_config(self) -> CacheConfig:
        ...
        result = {
            ...
            'window_size': cache_dict.get('window_strategy', {}).get('window_size',
    ...
    def get_credential(self, key: str, default: Any = None) -> Any:
        """获取credentials.yml中的配置值
        
        Args:
            key: 配置键，支持点号分隔的嵌套键（如 'akshare.ut'）
            default: 默认值
        ...
        Examples:
            >>> cm = ConfigManager()
            >>> # 获取 akshare 的 ut token
            >>> ut = cm.get_credential('akshare.ut')
            >>> # 获取 mock_provider
            >>> provider = cm.get_credential('mock_provider')
            >>> # 获取 tushare 的 token
            >>> token = cm.get_credential('tushare.token')
            >>> # 获取不存在的值，返回默认值
            >>> value = cm.get_credential('nonexistent.key', 'default_value')
            >>> # 获取多层嵌套的值
            >>> value = cm.get_credential('level1.level2.level3.key')
        """
        # credentials.yml 的内容加载到 self._config['credentials'] 下
        # 因为 ConfigManager 把每个 .yml 文件的内容放到对应的键下
    ...
    def set_credential(self, key: str, value: Any) -> bool:
        """设置credentials.yml中的配置值
        ...
        Examples:
            >>> cm = ConfigManager()
            >>> # 设置 akshare 的 ut token（仅内存）
            >>> cm.set_credential('akshare.ut', '7eea3edcaed734bea9cbfc24409ed989')
            >>> # 设置并持久化到文件
            >>> cm.set_credential('akshare.ut', '7eea3edcaed734bea9cbfc24409ed989', persist=True)
            >>> # 设置 mock_provider
            >>> cm.set_credential('mock_provider', 'test_credential', persist=True)
        """
        try:
            import yaml
            # 获取credentials.yml的路径
            config_path = self.get_config_path('credentials')
            # 读取现有内容并更新
            ...
            with open(config_path, 'w', encoding='utf-8') as f:
                yaml.safe_dump(existing_credentials, f, allow_unicode=True, default_flow_style=False)
