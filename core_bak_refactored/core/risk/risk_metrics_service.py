...
class RiskMetricsEngine(Protocol):
    def calculate_volatility(self, returns: pd.Series, window: Optional[int] = None, annualize: bool = True) -> float: ...
    def calculate_value_at_risk(self, returns: pd.Series, confidence_level: float = 0.95, method: str = 'historical') -> float: ...
    ...
    def calculate_alpha(self, asset_returns: pd.Series, market_returns: pd.Series, beta: Optional[float] = None) -> float: ...
    def calculate_calmar_ratio(self, returns: pd.Series) -> float: ...
    def calculate_all_metrics(self, asset_returns: pd.Series, market_returns: Optional[pd.Series] = None) -> Dict[str, float]: ...
