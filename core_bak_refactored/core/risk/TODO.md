...
**5A. 组合风险分析器** 🛠️ **P0优化完成**
- **文件**: `portfolio_risk.py` (736行)
- **状态**: 第16轮专家评审完成 + P0优化实施完成 ✅
- **专家评分**: 优秀（技术架构合理，具备生产环境部署基础）
- **依赖**: RiskMetricsService, StatisticalCalculator, ParallelExecutor, CacheService
- **测试**: 8/8测试通过
- **优化成果** (完成时间: 2024-11-21):
  - ✅ P0-1: 并行计算内存优化 (共享配置+进程级缓存，预计20-30%性能提升)
  - ✅ P0-2: 因子模型数值稳定性 (条件数检查+Ridge回归降级)
  - ✅ P0-3: 因子模型缓存集成 (预计50-70%计算加速)
- **下一步**: 性能基准测试验证优化效果
...
#### 🧭 迭代计划（基于专家 answer.md 提取的待落地项）

- P0-1 并行与内存优化（本迭代内继续完善）
  - [ ] 共享配置最小化：在 [`portfolio_risk.py`](portfolio_risk.py) 的并行打包路径强制使用最小配置子集 `_prepare_shared_config`
  - [ ] 进程级缓存生命周期：`WorkerAnalyzerCache` 增加内存压力感知清理（系统内存>85%触发）
  - [ ] 性能基准测试：补充并行加速比与命中率基线测试（目标3–5x / ≥85%）

- P0-2 因子模型稳定性（按市场与样本量动态化）
  - [ ] 动态阈值：实现 `_get_dynamic_thresholds(market_type, n_obs)` 并用于载荷估计
  - [ ] Ridge影响评估：实现 `_assess_ridge_impact` 并对 `max_beta_change > 0.5` 告警
  - [ ] 降级决策日志：记录条件数、阈值、alpha、影响评估结果

- P0-3 因子模型缓存（保持TTL=3600s）
  - [ ] 缓存键粒度优化：提升命中率到≥85%
  - [ ] 预研分层缓存原型（L1→L2[Redis]→L3[磁盘]）

- P1-1 动态分块算法（数据尺寸驱动）
  - [ ] 数据尺寸估算：在执行器加入 `_estimate_data_size_mb`（采样+fallback）
  - [ ] 分块策略：基于数据尺寸与CPU/内存计算最优 chunk_size，日志包含 data_size 提示
  - [ ] 任务类型自适应：预留 `_get_optimal_chunk_size_by_task_type`

- P1-2/P1-3 性能监控与诊断
  - [ ] 扩展指标：采集 `task_times`、`serialization_overhead`、`gil_contention`
  - [ ] 诊断阈值分层：按市场配置 R²/异方差/偏度/自相关阈值
  - [ ] 监控持久化：预研时序数据库接入（InfluxDB）与基线计算

- 配置参数外部化（P1.5）
  - [ ] 将关键阈值提取至配置（并行/因子模型），减少硬编码

- 里程碑与评审
  - [ ] 准备“第17轮专家评审”材料（围绕P0+P1优化成果与基线数据）
