...
class StatisticalCalculator:
    ...
    def calculate_percentile(
        ...
        Args:
            values: 数值序列
            percentile: 百分位点 (0-100)
            method: 插值方法（numpy 1.22+使用method代替interpolation）
        
        Returns:
            百分位数值
        
        示例:
            p5 = StatisticalCalculator.calculate_percentile(values, 5)
            p95 = StatisticalCalculator.calculate_percentile(values, 95)
        
        Note:
            与calculate_quantile的区别：
            - calculate_percentile: 输入0-100
            - calculate_quantile: 输入0-1
        """
        if values is None or len(values) == 0:
            return 0.0
