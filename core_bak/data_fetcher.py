...
class DataFetcher:
    ...
    支持依赖注入：
    - custom_sources: Dict[str, Callable] - 外部注入的数据源（用于测试Mock或自定义源）
    """

    def __init__(self, config: Dict, custom_sources: Optional[Dict[str, Any]] = None):
        self.config = config
        ...
        self.custom_sources = custom_sources or {}  # 外部注入的数据源
    ...
    def _initialize_data_sources(self) -> Dict:
        ...
        source_mapping = {
            DataSourceType.YAHOO_FINANCE.value: '_fetch_yahoo_data',
            DataSourceType.ALPHA_VANTAGE.value: '_fetch_alpha_vantage_data',
            DataSourceType.IEX_CLOUD.value: '_fetch_iex_cloud_data',
            DataSourceType.POLYGON.value: '_fetch_polygon_data',
            DataSourceType.TWELVE_DATA.value: '_fetch_twelve_data',
            DataSourceType.FINNHUB.value: '_fetch_finnhub_data',
            DataSourceType.TIINGO.value: '_fetch_tiingo_data',
            DataSourceType.QUANDL.value: '_fetch_quandl_data',
            DataSourceType.INTRINIO.value: '_fetch_intrinio_data',
            DataSourceType.EOD_HISTORICAL.value: '_fetch_eod_historical_data',
            DataSourceType.CUSTOM_API.value: '_fetch_custom_api_data',
            DataSourceType.DATABASE.value: '_fetch_database_data',
            DataSourceType.BROKER_API.value: '_fetch_broker_api_data',
        }

        # 只注册已实现的方法（避免AttributeError）
        for source_type, method_name in source_mapping.items():
            if source_type not in sources and hasattr(self, method_name):
                sources[source_type] = getattr(self, method_name)
            elif source_type not in sources:
                logger.debug(f"数据源 {source_type} 未实现，跳过注册")

        logger.info(f"已注册数据源: {list(sources.keys())}")
        return sources
...
# 数据质量监控器 (基础版)
# TODO: 专家提供的第1版实现,功能较基础,已被第2版(增强版)取代
# ============================================================================
# 数据质量监控器 (基础版) - 已废弃 ⚠️
# 
# .. deprecated:: 2025-11-29
#     建议使用 `core_bak_refactored.core.data.quality.DataQualityChecker` 替代
#     
#     迁移理由：
#     1. 职责分离：质量检查应独立于数据获取
#     2. 数据结构：DataFrame比List[MarketData]更通用
#     3. 功能整合：新版整合了多版本最优特性
#     
#     迁移示例：
#     ```python
#     # 旧代码
#     from core_bak_refactored.core.data.data_fetcher import DataQualityMonitorBasic
#     monitor = DataQualityMonitorBasic(config)
#     report = monitor.monitor_data_quality(market_data_list)
#     
#     # 新代码
#     from core_bak_refactored.core.data.quality import DataQualityChecker
#     from core_bak_refactored.core.data.converters import market_data_to_dataframe
#     
...
class DataQualityMonitorBasic:
    """数据质量监控器 (基础版) - 实时监控数据质量
    
    注意: 这是专家提供的第1版实现,功能较基础
    建议使用DataQualityMonitor(增强版)替代
    """
    ...
    def _setup_data_validation(self) -> Dict[str, Any]:
        ...
        return {
            'price_validation': {
                ...
                'high_low_consistency': True  # TODO: 补充缺失的配置项（测试驱动发现）
            },
            'volume_validation': {
                'min_volume': 0,
                'max_volume': 1e9,
                'volume_spike_threshold': 10.0,
                'volume_price_correlation': True  # TODO: 补充缺失的配置项（测试驱动发现）
            },
            'temporal_validation': {
    ...
    def _get_data_time_period(self, data: List[MarketData]) -> Dict[str, str]:
        """获取数据时间范围
        
        TODO: 补充专家完整版缺失的辅助方法（测试驱动发现）
        """
        if not data:
            return {'start': 'N/A', 'end': 'N/A', 'duration': 'N/A'}
        
        timestamps = [d.timestamp for d in data if hasattr(d, 'timestamp') and d.timestamp is not None]
        if not timestamps:
            return {'start': 'N/A', 'end': 'N/A', 'duration': 'N/A'}
    ...
    def _get_processing_statistics(self, data: List[MarketData]) -> Dict[str, Any]:
        """获取处理统计信息
        ...
        symbols = [getattr(d, 'symbol', None) for d in data if hasattr(d, 'symbol')]
        symbols = [s for s in symbols if s is not None]
        volumes = [d.volume for d in data if hasattr(d, 'volume') and d.volume is not None]
        prices = [d.close for d in data if hasattr(d, 'close') and d.close is not None]
        
        return {
            'total_records': len([d for d in data if hasattr(d, 'timestamp')]),
    ...
    def _calculate_expected_data_points(self, data: List[MarketData], expected_interval_hours: float = None) -> int:
        """计算预期数据点数量（支持自动识别频率，参数可覆盖）
        
        TODO: 补充专家完整版缺失的辅助方法（测试驱动发现）；依据 docs/answer.md 业务确认：多频率支持
        """
        if not data:
            return 0
        ...
        timestamps = [d.timestamp for d in data if hasattr(d, 'timestamp') and d.timestamp is not None]
        if not timestamps:
            return len([d for d in data if hasattr(d, 'timestamp')])
        start_time = min(timestamps)
        end_time = max(timestamps)
        ...
        if expected_interval_hours is None:
            freq = self._detect_data_frequency(data)
            expected_interval_hours = self._map_frequency_to_interval(freq)
        
        expected_points = max(int(duration_hours / expected_interval_hours) + 1, len(timestamps))
        return expected_points

    def _check_field_completeness(self, data: List[MarketData]) -> Dict[str, float]:
        """检查字段完整性
        ...
        # 检查必要字段（依据 docs/answer.md 建议扩展）
        required_fields = ['open', 'high', 'low', 'close', 'volume', 'adj_close', 'turnover', 'vwap']
        
        for field in required_fields:
            non_null_count = sum(1 for d in data if getattr(d, field, None) is not None)
            field_completeness[field] = non_null_count / total_count if total_count > 0 else 0.0
        
        return field_completeness

    def _detect_data_frequency(self, data: List[MarketData]) -> str:
        """自动检测数据频率（daily/hourly/minute/custom）"""
        timestamps = [d.timestamp for d in data if hasattr(d, 'timestamp') and d.timestamp is not None]
        if len(timestamps) < 2:
            return 'daily'
        timestamps.sort()
    ...
    def _map_frequency_to_interval(self, freq: str) -> float:
        """根据频率返回预期间隔（小时）"""
        intervals = {
            'daily': 24.0,
            'hourly': 1.0,
    ...
    def _detect_market_type(self, data: List[MarketData]) -> MarketCode:
        """检测市场类型（优先使用metadata枚举，其次symbol后缀启发式）
        
        Returns:
            MarketCode: 识别的市场枚举，无法识别时返回 MarketCode.UNKNOWN
        """
        if not data:
            return MarketCode.UNKNOWN
        meta = getattr(data[0], 'metadata', {}) or {}
        mt = meta.get('market_type')
        # 如果metadata中已经是枚举
        if isinstance(mt, MarketCode):
            return mt
        # 如果是字符串且有效，转换为枚举
        if isinstance(mt, str) and MarketCode.is_valid(mt):
            return MarketCode(mt)
        # 启发式：根据symbol后缀判断
        symbol = getattr(data[0], 'symbol', '') or ''
        if symbol.endswith('.US'):
            return MarketCode.US
        if symbol.endswith('.HK') or symbol.endswith('.HKG'):
            return MarketCode.HK
        if symbol.endswith('.SH') or symbol.endswith('.SZ') or symbol.endswith('.CN'):
            return MarketCode.CN
        return MarketCode.UNKNOWN

    def _get_gap_threshold(self, freq: str, market: MarketCode) -> float:
        """基于频率与市场的动态时间间隙阈值（小时）"""
        base_thresholds = {
            'daily': {MarketCode.US: 72.0, MarketCode.CN: 240.0, MarketCode.HK: 96.0, MarketCode.UNKNOWN: 48.0},
            'hourly': {MarketCode.UNKNOWN: 4.0},
            'minute': {MarketCode.UNKNOWN: 0.1},
            'custom': {MarketCode.UNKNOWN: 48.0}
        }
        market_thresholds = base_thresholds.get(freq, base_thresholds['daily'])
        return float(market_thresholds.get(market, market_thresholds[MarketCode.UNKNOWN]))

    def _check_time_continuity(self, data: List[MarketData], max_gap_hours: float = None) -> List[Dict]:
        """检查时间连续性（支持动态阈值，参数可覆盖）
        
        TODO: 补充专家完整版缺失的辅助方法（测试驱动发现）；依据 docs/answer.md 业务确认：动态阈值
        """
        gaps = []
        ...
        # 动态阈值：基于数据频率与市场类型
        freq = self._detect_data_frequency(data)
        market = self._detect_market_type(data)
        default_threshold = self._get_gap_threshold(freq, market)
        
        for i in range(1, len(sorted_data)):
            time_gap_hours = (sorted_data[i].timestamp - sorted_data[i-1].timestamp).total_seconds() / 3600
            threshold = max_gap_hours if max_gap_hours is not None else default_threshold
    ...
    def _generate_recommendations(self, quality_report: Dict) -> List[Dict]:
        ...
        if quality_report['anomalies_detected']:
            ...
            recommendations.append({
                ...
                'action': '调查检测到的数据异常',
                'details': f'发现 {anomaly_count} 个数据异常'
            })
        ...
        if quality_report['overall_score'] < 0.8:
            ...
            recommendations.append({
                ...
                'category': 'overall',
                'action': '全面检查数据质量流程',
                'details': f'总体数据质量评分较低: {overall_score:.3f}'
            })
...
# 
# .. deprecated:: 2025-11-29
#     建议使用 `core_bak_refactored.core.data.quality.DataQualityChecker` +
#     `core_bak_refactored.app.data.monitoring_service.MonitoringService` 组合
#     
#     迁移理由：
#     1. 实时监控应在应用层(MonitoringService)，而非核心层
#     2. 性能统计应由独立监控服务管理
#     3. ML异常检测可选性更强，避免核心依赖
#     
#     迁移示例：
#     ```python
#     # 旧代码
...
#     monitor.start_monitoring()  # 启动后台线程
#     
#     # 新代码
...
#     monitoring_service.start()  # 应用层监控编排
#     ```
# ============================================================================
class DataQualityMonitor:
    """数据质量监控器 (增强版) - 实时监控数据质量
