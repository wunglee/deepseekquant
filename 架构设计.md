...
│                   业务层 (Business Layer)                     │
├─────────────────────────────────────────────────────────────┤
│ 信号引擎     组合管理     执行引擎     风险引擎     数据分析     │
│ SignalEngine PortfMgr     ExecEngine  RiskEngine   Analytics  │
├─────────────────────────────────────────────────────────────┤
│                   服务层 (Service Layer)                      │
├─────────────────────────────────────────────────────────────┤
│ 数据服务     事件总线     缓存服务     配置服务     日志服务     │
│ DataService  EventBus     CacheService ConfigService LogService│
├─────────────────────────────────────────────────────────────┤
│                   核心层 (Core Layer)                         │
├─────────────────────────────────────────────────────────────┤
│ 基础处理器   组件管理     任务调度     资源管理     错误处理     │
│ BaseProcessor ComponentMgr TaskScheduler ResourceMgr ErrorHandler│
├─────────────────────────────────────────────────────────────┤
│                   基础设施层 (Infrastructure)                 │
├─────────────────────────────────────────────────────────────┤
│ 数据库       消息队列     文件存储     网络通信     计算引擎     │
│ Database     MessageQueue FileStorage  Network     ComputeEngine│
└─────────────────────────────────────────────────────────────┘
```
...
- 兼容性与导入策略
  - 优先使用 core.* 绝对导入；包内使用相对导入作为回退
  - 清理遗留的路径注入逻辑，测试侧去除 sys.path hack
- 后续建议（可选）
  - 在 processors/ 下按具体业务处理器进一步分包
  - 为 managers/ 增加接口抽象，便于替换实现
  - 为 components/ 增加更细粒度的单元测试覆盖