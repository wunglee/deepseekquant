...
### portfolio_risk.py
- 缺失或长度不一致的价格序列下，是否应进行权重重标化与稳健协方差估计，以保持归因稳定性？
- 组合VaR/CVaR呈现口径（绝对值/保留符号）如何与风控报表和回测报表保持一致性？

### position_risk.py
- 单仓VaR是否建议引入厚尾或跳跃分布模型以适配极端行情，并给出参数校准方法？
- 参与率冲击（α、β）是否需按市场与板块分层校准，并设置业务上限防过拟合？

### risk_monitor.py
- 告警分级是否应引入指标权重矩阵（VaR/MDD/TE等）与市场差异化阈值，以减少误报？
- 高并发情况下是否需要防抖/节流与熔断降级策略，避免告警风暴并保证处理有序？

### risk_models.py
- `RiskLevel` 与 `ImpactLevel` 的语义边界、单位与范围校验是否需要统一规范，避免跨团队口径差异？
- `from_dict` 容错策略是否需要统一审计字段与日志等级规范，保障生产稳健性？
