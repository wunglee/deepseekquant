...
1. **不要推测或补全**：如果问题描述不清晰或缺少信息，请直接指出，不要自行推测答案
2. **明确具体指导**：请提供可直接执行的代码示例或配置示例，避免抽象描述
3. **职责边界明确**：如果建议涉及其他模块（如执行、回测），请明确指出并提供跨模块协作方案
4. **参数来源说明**：如需使用外部数据（汇率、市场数据），请说明获取方式和数据格式
5. **优先级清晰**：请明确标注P0/P1/P2优先级，避免全部标记为重要
6. **可验证性**：建议应包含验证方法（测试用例、预期结果），便于确认实施正确性
