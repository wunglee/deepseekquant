...
> **版本**: v3.2 | **更新**: 2024-11-21 | **范围**: 架构约束、开发规范、工作流程、质量标准
...
- ✅ **自主决策**：优化过程AI自行决策，不得要求用户介入确认
- ✅ **报告要求**：完成报告中必须包含优化细节、技术理由、性能对比（如适用）
- ✅ **示例优化**：
  - 缓存策略优化（L1/L2分层、TTL调整、淘汰算法）
  - 并行计算优化（分块策略、worker池、序列化开销）
  - 数值稳定性（条件数阈值、Ridge降级、QR→伪逆回退）
  - 异常处理增强（分级日志、降级策略、错误恢复）
...
| 重构引入新特性 | 优化时添加业务逻辑 | 仅限内部技术优化 |
| 重构要求用户确认 | 优化方案询问用户 | AI自主决策，报告体现细节 |
| 直接照搬专家建议 | 未考虑架构实际 | 理解消化后落地，检查冗余 |
...
**版本**: v3.2 (2024-11-21) - 新增专家建议理解消化约束  
**状态**: ✅ 生效中
