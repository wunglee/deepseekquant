...
class RiskCalculator:
    ...
    def __init__(self, config: Dict[str, Any], metrics_engine: Optional[RiskMetricsEngine] = None):
        """
        初始化风险计算器（支持依赖注入）
        ...
        Args:
            ...
            metrics_engine: 可注入的风险指标引擎（Protocol）
                - 默认实现为 RiskMetricsService
                - 可传入 Mock/加速版/外部实现以支持不同运行环境
...
class PortfolioRiskAnalyzer:
    ...
    def __init__(
        ...
        metrics_engine: Optional[RiskMetricsEngine] = None
    ):
        """
        初始化组合风险分析器（支持依赖注入）
        ...
        Args:
            ...
            metrics_engine: 可注入的风险指标引擎（Protocol），默认 RiskMetricsService
        """
```
